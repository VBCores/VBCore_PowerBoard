...
volatile char uart_rx_buffer[UART_RX_BUFFER_SIZE + 1];
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef* huart, uint16_t size) {
    auto rx_accum = std::string(const_cast<char*>(uart_rx_buffer), size);
    if (rx_accum.find('\n') == std::string::npos) {
        start_uart_recv_it(huart, false);
    }
    else {
        process_command(rx_accum);
        start_uart_recv_it(huart, true);
    }
}

void start_uart_recv_it(UART_HandleTypeDef* huart, bool clear_buffer) {
    if (clear_buffer) {
        std::memset(const_cast<char*>(uart_rx_buffer), '\0', UART_RX_BUFFER_SIZE + 1);
    }
    ...
    */
    ...
    auto status = HAL_UARTEx_ReceiveToIdle_DMA(
        huart,
        reinterpret_cast<uint8_t*>(const_cast<char*>(uart_rx_buffer)),
        UART_RX_BUFFER_SIZE
    );
    if (status != HAL_OK) {
        // TODO
